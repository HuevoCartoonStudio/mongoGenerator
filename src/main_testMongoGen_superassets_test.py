...
if __name__ == "__main__":
	...
	for j in range(number_superassets):
		...
		for i in range(numbersito):
			list_id_subscript_itemes.append(randint(0, number_assets))

		for k in list_id_subscript_itemes:
			itemp_temp = items_dao.readOneByProperty(items_dao.Properties.Name, 'Asset_' + str(k))
			if itemp_temp is not None:
				list_subscript_itemes.append(itemp_temp)
			# print (item_temp.getId())

		for l in list_subscript_itemes:
			list_items[j].addAssetSubscription(l.getId())
			l.addSuperAssetSubscription(list_items[j].getId())
			items_dao.update(l)
			# print('hello' + str(l))
